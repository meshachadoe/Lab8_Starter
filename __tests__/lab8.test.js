describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    const shadowRootElem = await shadowRoot.asElement();

    // Once you have the button, you can click it and check the innerText property of the button.
    const button = await shadowRootElem.$('button');
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    const prodItems = await page.$$('product-item');
    for (let i = 1; i < 20; i++) {
      const shadowRootElem = await (await prodItems[i].getProperty('shadowRoot')).asElement();
      const button = await shadowRootElem.$('button');
      await button.click()
    }
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for (let i = 0; i < 20; i++) {
      const shadowRootElem = await (await prodItems[i].getProperty('shadowRoot')).asElement();
      const buttonText = await (await shadowRootElem.$('button')).getProperty('innerText');
      expect(buttonText['_remoteObject'].value).toBe('Remove from Cart')
    }
